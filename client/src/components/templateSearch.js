        ...
        <section className="template-search">
            ...
            <div className="template-search-viewport">
                <ul className="template-search-list">
                    {temp.templates.map((elem, i) => {
                        return <li 
                                    className={`template-search-item ${templateObj.id == elem.id ? 'selected' : ''}` } 
            ...
            <div className="template-search-details">
                <div className={`wrapper${(templateObj == 0) ? " hidden" : ""}`}>
                    ...
                    <p><strong>Difficulty:</strong> {templateObj.difficulty_rating}</p>
                    <hr />
                    <h2>Recommendations</h2>
                    <p><strong>Expected duration:</strong> {templateObj.time_esitmate}</p>
                    <p><strong>Team size:</strong> {templateObj.team_estimate}</p>
                </div>
            </div>
            <footer>
                <Button disabled={(templateObj == 0) ? "disabled" : ""}>Assemble project</Button>
            </footer>
        </section>
    );
